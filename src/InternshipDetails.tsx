...
const parseDescription = (text: string) => ({
  ...
  company: extractSection(text, "ABOUT THE COMPANY"),
});
...
function InternshipDetails() {
  ...
  try {
    const res = await fetch(
      `http://localhost:3000/internships/apply/${id}`,
      {
        method: "POST",
        headers: {
...
>
    {saved ? "Saved ✓" : "Save Internship"}
        ...
        <div className="details-card">
          ...
          <ul className="bullet-list">
            {toBulletPoints(sections.company).map((item, i) => (
              <li key={i}>{item}</li>
            ))}
          </ul>
        </div>
